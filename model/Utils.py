...
def get_config(attribute):
    ...
    if attribute in __cfg:
        ...
        raise ValueError(f"Unknown config attribute: '{attribute}'")
...
def loadWomanNames():
    """! Loads the bee names from 'Names/Vornamen_2018_Koeln.csv' and returns them as list
    """
    global _woman_names
    if type(_woman_names) == type(None):
        _woman_names = []
        script_dir = os.path.dirname(os.path.abspath(__file__))
        names_path = os.path.join(script_dir, 'Names', 'Vornamen_2018_Koeln.csv')
        with open(names_path, encoding="utf8", errors="ignore") as _file:
